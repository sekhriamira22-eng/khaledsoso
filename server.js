...
// ★ API السلة اليومية — يقرأ من message_buffer
    ...
    try {
        ...
        for (const row of result.rows) {
            ...
            for (const line of lines) {
                ...
                // فلترة حسب الباراميترات
                if (date && lineDate !== date) continue;
                if (tailor && t !== tailor) continue;
                if (clothes_type && ct !== clothes_type) continue;
                ...
                // استخراج inspector/batch_qty/defects من notes
                if (notes) {
                    const inspMatch = notes.match(/عامل الفحص:\s*([^|،\n]+)/);
                    const bqMatch   = notes.match(/كمية الدفعة:\s*([^|،\n]+)/);
                    const dfMatch   = notes.match(/الأعطاب:\s*([^|،\n]+)/);
                    if (inspMatch && !summary[key].inspector) summary[key].inspector = inspMatch[1].trim();
                    if (bqMatch   && !summary[key].batch_qty)  summary[key].batch_qty  = bqMatch[1].trim();
                    if (dfMatch   && !summary[key].defects)    summary[key].defects    = dfMatch[1].trim();
                }
            }
        }
        res.json({ data: Object.values(summary) });
    } catch (e) {
        console.error('❌ /api/daily-basket error:', e.message);
